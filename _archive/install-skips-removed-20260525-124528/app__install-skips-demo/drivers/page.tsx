    ...
    <main className="min-h-screen install-skip-bg text-white">
      ...
      <section className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.075] p-6">
          <p className="text-sm font-black uppercase tracking-[0.28em] text-yellow-300">Driver Phone Tasks</p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {tasks.map(([driver, skip, job, note, taco]) => (
            <article key={`${driver}-${skip}`} className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-5">
              <p className="text-sm font-black text-yellow-300">{driver}</p>
              ...
              <div className="mt-4 rounded-2xl bg-black/25 p-4">
                <p className="text-xs font-bold uppercase tracking-widest text-slate-300">TACO / Hours</p>
                <p className="mt-2 text-sm font-black">{taco} · Break reminder active</p>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href="/install-skips-demo/skip-ticket" className="rounded-2xl bg-yellow-300 px-4 py-3 text-sm font-black text-black">Open Ticket</Link>
