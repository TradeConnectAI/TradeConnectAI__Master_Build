...
const liveOps = [
  ["SCANIA 340 delivering Barry", "green"],
  ["2 collections due today", "blue"],
  ["£645 paid today", "green"],
  ["2 jobs sent to phones", "purple"],
];

function Logo() {
  return (
    <div className="flex items-center gap-3">
      <div className="grid h-11 w-11 place-items-center rounded-full bg-orange-600 text-xl font-black text-black">
        C
      </div>
      <div className="leading-none">
        <p className="text-2xl font-black tracking-tight text-white">INSTALL</p>
        <p className="mt-1 text-xs font-black uppercase tracking-[0.18em] text-orange-500">
          Integrated Skips
        </p>
      </div>
    </div>
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      <div className="grid min-h-screen lg:grid-cols-[260px_1fr]">
        <aside className="border-b border-white/10 bg-[#060708] lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
              ...
              <nav className="mt-8 flex gap-2 overflow-x-auto pb-2 lg:flex-col lg:overflow-visible">
                {sidebar.map((item, index) => (
                  <a
                    ...
                    className={
                      index === 0
                        ? "whitespace-nowrap rounded-xl bg-orange-600 px-4 py-3 text-sm font-black text-white"
                        : "whitespace-nowrap rounded-xl px-4 py-3 text-sm font-semibold text-slate-300 hover:bg-white/10"
                    }
                  >
                    {item}
                  </a>
        ...
        <section className="min-w-0 px-4 py-5 sm:px-6 lg:px-8">
          <header className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="grid grid-cols-2 gap-2 sm:flex">
              <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-bold">
                22 May 2025
              </div>
              <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-bold">
                08:20
              </div>
              <button className="col-span-2 rounded-xl bg-orange-600 px-6 py-3 text-sm font-black text-white sm:col-span-1">
                + New Job
              </button>
            </div>
          </header>

          <section className="mt-6 overflow-hidden rounded-3xl border border-white/10 bg-slate-950">
            <div className="relative min-h-[290px] bg-[radial-gradient(circle_at_75%_35%,rgba(249,115,22,0.45),transparent_28%),linear-gradient(90deg,rgba(0,0,0,0.92),rgba(0,0,0,0.62)),linear-gradient(135deg,#1f2937,#020617)] p-5 sm:p-8">
              <div className="absolute right-8 top-8 hidden rounded-3xl border border-orange-500/20 bg-orange-600/20 px-10 py-8 text-7xl font-black text-orange-500/50 xl:block">
                INSTALL
              </div>
              ...
              <div className="relative z-10 max-w-3xl">
                ...
                <p className="mt-5 max-w-2xl text-sm leading-6 text-slate-200 sm:text-base">
                  A clickable private demo for Install Integrated Skips. AI answers
                  enquiries, checks availability, organises permits, assigns drivers,
                  sends task to phones, takes payments and hands complex work to a real person.
                </p>
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {["Book Skip ??", "Driver Board", "Collections", "Live Map", "£ Revenue"].map((item) => (
                    <a
                      key={item}
                      href="#"
                      className="rounded-full border border-orange-500/70 bg-black/45 px-4 py-2 text-sm font-black text-orange-400"
                    >
                      {item}
                    </a>
                  ))}
          ...
          <section className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {stats.map(([label, value, note]) => (
              ...
              >
                ...
                <p className="mt-1 text-sm font-medium text-orange-100">{note}</p>
              </div>
            ))}
          </section>
          ...
          <section className="mt-5 rounded-2xl border border-orange-500/20 bg-white/[0.02] p-4">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
              {liveOps.map(([text, colour]) => (
                <div key={text} className="rounded-xl bg-black/40 p-3 text-sm font-semibold">
                  <span
                    className={
                      colour === "green"
                        ? "mr-2 inline-block h-3 w-3 rounded-full bg-green-400"
                        : colour === "blue"
                        ? "mr-2 inline-block h-3 w-3 rounded-full bg-blue-400"
                        : "mr-2 inline-block h-3 w-3 rounded-full bg-purple-400"
                    }
                  />
                  {text}
                </div>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={
                  tab === "Payments"
                    ? "whitespace-nowrap rounded-xl bg-orange-600 px-4 py-3 text-sm font-black text-white"
                    : "whitespace-nowrap rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm font-bold text-white"
                }
              >
                {tab}
              </button>
          ...
          <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <div className="mt-5 space-y-3">
                {payments.map(([initials, name, invoice, status, amount, due]) => (
                  <div
                    key={invoice}
                    className="flex items-center justify-between gap-4 rounded-2xl bg-black/35 p-4"
                  >
                    <div className="flex items-center gap-3">
                      <div className="grid h-11 w-11 place-items-center rounded-full bg-orange-600 text-sm font-black text-white">
                        {initials}
                      </div>
                      <div>
            ...
            <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]">
              ...
              <div className="relative min-h-[260px] bg-[radial-gradient(circle_at_22%_72%,rgba(34,197,94,0.5),transparent_10%),radial-gradient(circle_at_45%_37%,rgba(249,115,22,0.75),transparent_8%),radial-gradient(circle_at_60%_35%,rgba(249,115,22,0.75),transparent_8%),radial-gradient(circle_at_82%_36%,rgba(14,165,233,0.35),transparent_22%),linear-gradient(135deg,#164e63,#052e16_42%,#0f172a)]">
                <div className="absolute left-[20%] top-[68%] rounded-full bg-green-500 px-3 py-2 text-xs font-black">
                  Barry
                </div>
                <div className="absolute left-[42%] top-[32%] rounded-full bg-orange-600 px-3 py-2 text-xs font-black">
                  Phone
                </div>
                <div className="absolute left-[56%] top-[34%] rounded-full bg-orange-600 px-3 py-2 text-xs font-black">
                  Dinas
                </div>
                ...
                <article key={ref} className="rounded-2xl bg-black/35 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-1 text-lg font-black">{customer}</h3>
                    </div>
...
try {
  ...
  execSync('git commit -m "Upgrade install skips demo control room layout"', { stdio: "inherit" });
...
console.log("DONE. Clean demo URL remains /install-skips-demo");
