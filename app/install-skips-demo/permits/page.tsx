    ...
    <main className="min-h-screen bg-[#06110d] text-white">
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-6">
          <p className="text-sm font-black uppercase tracking-[0.28em] text-emerald-300">Permit Checks</p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={place} className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5">
              <h2 className="text-xl font-black">{place}</h2>
              <p className="mt-2 text-sm font-bold text-emerald-300">{result}</p>
              <p className="mt-2 text-sm text-slate-300">{note}</p>
            </div>
          ))}
        </div>

        <Link href="/install-skips-demo/book" className="mt-6 inline-flex rounded-2xl bg-emerald-400 px-5 py-4 text-sm font-black text-black">
          Back to Booking
        </Link>
      </section>
    </main>
